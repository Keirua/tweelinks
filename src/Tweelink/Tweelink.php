...
class Tweelink {
    ...
    public function cacheNewTweets () {
        ...
        */
        if ($code == 200) {
            $tweets = json_decode($this->tmhOAuth->response['response']);
                ...
                try {
                    ...
                    foreach ($tweets as $tweet) {
                        ...
                        for ($i = 0; $i < count ($tweet->entities->urls); ++$i)  {
                            ...
                            $currUrl = $this->getFinalUrl($tweet->entities->urls[$i]->expanded_url);
                            $title = addslashes (trim($this->getTitle ($currUrl)));
